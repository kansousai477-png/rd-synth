...
CFG = {
    ...
    # --------------------------
    # BASELINE ARTIFACT (canonical cols + shared std)
    # --------------------------
    "baseline": {
        # MUST contain meta.json + standardizer.npz
        "artifact_dir": "artifacts/baselines_light_nb15_v2",
        # baseline exported NPZs here
        "export_subdir": "adv_npz",
        # baseline_zoo config uses "NB15_" in your post
        "export_prefix": "NB15_",
        # if you left prefix empty in some runs, keep it "".
        # script will auto-discover if mismatch.
    },
    ...
    "metrics": {
        ...
        # Outputs
        "out_dir": "results/metrics_suite_v3_unsw_integrated",
        "save_txt": True,
    },

    # --------------------------
    # EXTRA generated sources (manual override, e.g., RD-Synth)
    # --------------------------
    "extra_gen_sources": {
...
# ============================================================
# STP split for UNSW-like flow features
# ============================================================
def split_feature_blocks_unsw(cols: List[str]):
    ...
    time_keys = [
        "duration", "iat", "flow_duration", "active", "idle",
        "flow_iat", "fwd_iat", "bwd_iat",
        "start_time", "end_time",
    ]
    size_keys = [
        "packet", "length", "bytes", "pkts", "packets",
        "total_length", "totlen", "payload",
        "avg", "mean", "min", "max", "std",
        "rate", "bytes/s", "packets/s",
    ]
    proto_keys = [
        "port", "protocol", "flag", "tcp", "udp", "icmp",
...
def load_manifest_sources(manifest_path: str) -> dict:
    ...
    for k, v in paths.items():
        ...
        cand1 = os.path.normpath(os.path.join(base_dir, p))
        if _exists(cand1):
        ...
        # last resort: still return a sensible absolute guess
        out[str(k)] = os.path.abspath(cand1)
...
def evaluate_one_method(X_real_std: np.ndarray, X_gen_std: np.ndarray, idxT, idxS, idxP, cfgm) -> dict:
    ...
    auc, acc = c2st_metrics(
        Xr, Xg,
        seed=int(cfgm["c2st_seed"]),
        test_size=float(cfgm["c2st_test_size"]),
        max_iter=int(cfgm["c2st_max_iter"])
    )
    out["C2ST-AUC"] = auc
    out["C2ST-Acc"] = acc
...
def find_manifest_path(artifact_dir: str, export_subdir: str, export_prefix: str) -> str:
    ...
    # 2) list all manifest candidates
    cands = sorted(glob.glob(os.path.join(out_dir, "*manifest.json")))
    if len(cands) == 0:
        raise FileNotFoundError(
            f"No '*manifest.json' found in {out_dir}. "
            f"Did baseline_zoo export adv_npz correctly?"
        )
    ...
    if export_prefix:
        ...
        if len(hits) > 1:
            raise FileNotFoundError(
                f"Multiple prefix-matched manifests under {out_dir} for prefix='{export_prefix}':\n"
                + "\n".join(hits)
                + "\nPlease set CFG['baseline']['export_prefix'] more specifically."
            )

    raise FileNotFoundError(
        f"Multiple manifests found under {out_dir}:\n"
        + "\n".join(cands)
        + "\nPlease set CFG['baseline']['export_prefix'] to choose one, "
          "or delete/rename old manifests."
    )


# ============================================================
# Run metrics (integrated)
# ============================================================
def run_metrics(cfg_all: dict, manifest_path: str, extra_gen_sources: Optional[dict] = None) -> None:
    ...
    benign_value = int(cfg_all.get("benign_value", 0))

    # Load baseline shared std + meta.cols (canonical)
    ...
    if not os.path.exists(meta_path) or not os.path.exists(std_path):
        raise FileNotFoundError(
            f"Missing baseline meta.json or standardizer.npz.\n"
            f"meta: {meta_path}\nstd : {std_path}"
        )
    ...
    feat_cols = norm_cols(meta_cols)  # ✅ canonical cols
    print(f"[ColsCanon] Metrics uses baseline meta.cols: D={len(feat_cols)}")

    std = Standardizer.load(std_path)
    if len(std.mu) != len(feat_cols):
        raise ValueError(f"[Std] Dim mismatch: len(std.mu)={len(std.mu)} vs len(meta.cols)={len(feat_cols)}. Wrong artifact_dir?")

    print(f"[Std] Loaded shared standardizer: D={len(std.mu)} from {std_path}")

    # Load real benign raw -> std (aligned to canonical cols)
    ...
    print("\n=== Metrics Table (INTEGRATED; shared standardizer space) ===")
...
def main():
    ...
    extra = CFG.get("extra_gen_sources", None)
