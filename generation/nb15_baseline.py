...
# ============================================================
CFG = {
    # UNSW-NB15 preprocessed CSV (binary Label)
    ...
    # ✅ aligned artifact dir name (so nb15 DDPM can find meta.json)
...
def np_savez_xadv(
    path: str,
    Xadv_raw: np.ndarray,
    Xadv_std: Optional[np.ndarray],
    extra: Optional[dict] = None
) -> None:
    """
    Save NPZ in a loader-friendly way.

    ✅ Always save:
      - Xadv_raw : raw feature space (preferred key for metrics suite)
      - Xadv     : raw feature space (backward compatibility alias)

    Optional:
      - Xadv_std : standardized space under saved standardizer
    """
    extra = extra or {}
    ensure_dir(os.path.dirname(path) or ".")

    Xraw = np.asarray(Xadv_raw, dtype=np.float32)
    payload = {
        "Xadv_raw": Xraw,
        "Xadv": Xraw,  # backward compatible alias
    }
    if Xadv_std is not None:
        payload["Xadv_std"] = np.asarray(Xadv_std, dtype=np.float32)
...
# ============================================================
# UNSW loader (chunked sampling + schema lock)
# ============================================================
...
def load_and_split_single_nb15(
    csv_path: str,
    n_ben: int,
    n_mal: int,
    label_col: str = "Label",
    ...
    """
    print(f"[Info] Loading NB15 (chunked): {csv_path}")
...
class BaselineZoo:
    ...
    def export_adv_npz(self, method: str, Xm_raw: np.ndarray, out_path: str, batch_size: int = 1024, **kwargs) -> str:
        ...
        extra = {
            ...
            "cfg_fingerprint": np.array([self.meta.get("cfg_fingerprint", "")], dtype=str),
            "std_fit_on": np.array([str(self.cfg.get("std_fit_on", "unknown"))], dtype=str),
            "std_fingerprint": np.array([self.std.fingerprint()], dtype=str),
            "dataset": np.array([DATASET_NAME], dtype=str),
            "cols": np.array([str(c) for c in cols], dtype=str),
            "version": np.array(["v2.1-integrated"], dtype=str),
        }
    ...
    def export_all_adv(self, Xm_raw: np.ndarray, methods: List[str], out_subdir: str = "adv_npz", prefix: str = "", **kwargs) -> Dict[str, str]:
        ...
        manifest = {
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "artifact_dir": os.path.abspath(self.artifact_dir),
            ...
            "version": "v2.1-integrated",
            "note": (
                "NPZ contains Xadv_raw (raw). Also contains Xadv (alias). "
                "If export_include_std=True, also contains Xadv_std (standardized). "
                "Each NPZ includes ordered cols and std_fingerprint to verify alignment/scaler."
            ),
        }

        # ✅ Write BOTH manifests:
        #   1) manifest.json           (prefix-free, metrics suite default)
        #   2) {prefix}manifest.json   (keep your original naming)
        manifest1 = os.path.join(out_dir, "manifest.json")
        manifest2 = os.path.join(out_dir, f"{prefix}manifest.json") if prefix else manifest1

        save_json(manifest, manifest1)
        if manifest2 != manifest1:
            save_json(manifest, manifest2)

        print(f"[Zoo] Exported NPZs + manifest to: {out_dir}")
        print(f"[Zoo] Manifest: {manifest1}")
        if manifest2 != manifest1:
            print(f"[Zoo] Manifest: {manifest2}")

        # Small directory summary (helps avoid “loaded yesterday’s npz”)
        try:
            files = sorted([f for f in os.listdir(out_dir) if f.lower().endswith((".npz", ".json"))])
            print("[Zoo] adv_npz contents:")
            for f in files:
                p = os.path.join(out_dir, f)
                ts = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(os.path.getmtime(p)))
                print(f"  - {f:28s}  (mtime: {ts})")
        except Exception:
            pass
...
def train_and_save_all(cfg: dict) -> BaselineZoo:
    ...
    Xb, Xm, cols = load_and_split_single_nb15(
        cfg["csv_path"],
        n_ben=cfg["n_ben"],
        n_mal=cfg["n_mal"],
