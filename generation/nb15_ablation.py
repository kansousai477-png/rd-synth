import os, json, time, random, warnings, datetime, hashlib, copy
...
CFG = {
    ...
    # output root
    "out_root": "results_nb15_rdsynth_ablation",
    ...
    # aux losses (FULL)
    "lambda_stp": 0.05,    # STP regularizer
    "lambda_corr": 0.001,  # correlation-matrix regularizer
    "lambda_mmt": 0.02,    # moment-matching regularizer
    "aux_prob": 0.50,
    ...
    # =====================================================
    # Ablation settings (edit names if you want paper-ready)
    # =====================================================
    "ablations": [
        # "full" == all regularizers on
        {"name": "full",       "use_stp": True,  "use_corr": True,  "use_mmt": True},
        # w/o lstp  -> interpret as w/o STP
        {"name": "w_o_stp",    "use_stp": False, "use_corr": True,  "use_mmt": True},
        {"name": "w_o_corr",   "use_stp": True,  "use_corr": False, "use_mmt": True},
        # w/o lmmt  -> w/o MMT
        {"name": "w_o_mmt",    "use_stp": True,  "use_corr": True,  "use_mmt": False},
        # w/o all   -> disable all aux
        {"name": "w_o_all",    "use_stp": False, "use_corr": False, "use_mmt": False},
    ],

    # if True: reuse the same sampled Xm_raw_gen across variants for fairness
    "share_gen_conditioning_across_variants": True,
}
...
def train_v2(Xb_tr, Xb_val, Xm, idxT, idxS, idxP, cfg):
    """
    cfg must include:
      lambda_stp, lambda_corr, lambda_mmt, aux_prob
    """
    ...
    lam_stp = float(cfg.get("lambda_stp", 0.0))
    lam_corr = float(cfg.get("lambda_corr", 0.0))
    lam_mmt = float(cfg.get("lambda_mmt", 0.0))
    aux_prob = float(cfg.get("aux_prob", 0.0))
    ...
    for ep in range(1, epochs + 1):
        ...
        for it, (xb, xm) in enumerate(zip(db, cycle(dm)), start=1):
            ...
            with torch.cuda.amp.autocast(enabled=amp_ok):
                ...
                # aux (only if any lambda > 0 and prob triggers)
                if (aux_prob > 0) and (rng.rand() < aux_prob) and ((lam_stp + lam_corr + lam_mmt) > 0):
                    a_bar_t = DDPM._extract(ddpm.a_bar, t_int, xb.shape)
                    x0_pred = (x_t - (1.0 - a_bar_t).sqrt() * eps_pred) / (a_bar_t.sqrt() + 1e-8)

                    L_stp = stp_loss_weighted(x0_pred, xb, idxT, idxS, idxP) if lam_stp > 0 else torch.tensor(0.0, device=device)
                    L_corr = corr_matrix_loss(x0_pred, xb) if lam_corr > 0 else torch.tensor(0.0, device=device)
                    L_mmt = moment_match_loss(x0_pred, xb, [idxT, idxS, idxP]) if lam_mmt > 0 else torch.tensor(0.0, device=device)
                else:
                ...
                loss = L_diff + lam_scale * (lam_stp * L_stp + lam_corr * L_corr + lam_mmt * L_mmt)
...
def cfg_for_variant(base_cfg: dict, variant: dict) -> dict:
    """Return a modified cfg for one ablation variant."""
    cfg = copy.deepcopy(base_cfg)
    use_stp = bool(variant.get("use_stp", True))
    use_corr = bool(variant.get("use_corr", True))
    use_mmt = bool(variant.get("use_mmt", True))
    ...
    # If all aux are off, also reduce aux_prob to 0 for clarity
    if (cfg["lambda_stp"] + cfg["lambda_corr"] + cfg["lambda_mmt"]) <= 0:
        cfg["aux_prob"] = 0.0

    return cfg


def save_npz(out_npz, Xadv_raw, Xadv_std, cols, method_name, dataset_name, std: Standardizer, std_path, ckpt_dir, chk):
    ensure_dir(os.path.dirname(out_npz) or ".")
    np.savez(
        out_npz,
        Xadv_raw=Xadv_raw,
        Xadv=Xadv_raw,              # ✅ raw alias
        Xadv_std=Xadv_std,
        cols=np.array([str(c) for c in cols], dtype=str),
        method=np.array([method_name], dtype=str),
        dataset=np.array([dataset_name], dtype=str),
        ...
        meta={
            ...
            "stdcheck_mean_abs": float(chk),
        }
    )


def run_one_variant(run_root, base_cfg, variant, cols, std, std_path, Xb_std, Xm_std_train, Xb_tr, Xb_val, idxT, idxS, idxP, Xm_raw_gen_shared=None):
    vname = str(variant["name"])
    vdir = ensure_dir(os.path.join(run_root, vname))
    print("\n" + "="*90)
    print(f"[Variant] {vname}")
    print("="*90)

    cfg_v = cfg_for_variant(base_cfg, variant)

    # train
    t0 = time.time()
    enc, eps, enc_ema, eps_ema, ddpm, df_log = train_v2(Xb_tr, Xb_val, Xm_std_train, idxT, idxS, idxP, cfg_v)
    print(f"[Info] Training done in {(time.time()-t0)/60:.1f} min | variant={vname}")

    # save training artifacts (and benign std range)
    ckpt_dir, meta = save_training_artifacts(vdir, enc, eps, enc_ema, eps_ema, ddpm, cols, idxT, idxS, idxP, df_log, cfg_v, std, Xb_std)
    ben_lo = np.array(meta["ben_std_min"], dtype=np.float32)
    ben_hi = np.array(meta["ben_std_max"], dtype=np.float32)

    # load malicious RAW for generation (optionally shared across variants)
    if Xm_raw_gen_shared is None:
        Xm_raw_gen = load_malicious_raw_for_gen(
            base_cfg["csv_path"], base_cfg["label_col"], feat_cols=cols,
            n_mal=int(base_cfg["n_mal_gen"]),
            seed=int(base_cfg["seed"]),
            chunksize=int(base_cfg["chunksize"]),
        )
    else:
        Xm_raw_gen = Xm_raw_gen_shared
    ...
    Xadv_std = ddpm_sample_conditional(enc_s, eps_s, ddpm, Xm_std_gen, cfg_v, ben_lo=ben_lo, ben_hi=ben_hi)
    ...
    print(f"[StdCheck] {vname}: mean|shared_std(Xadv_raw)-Xadv_std| = {chk:.8f} (should be ~0)")

    out_npz = os.path.join(vdir, f"rd_synth_adv_{vname}.npz")
    method_name = f"RD-Synth({vname})"
    dataset_name = "UNSW-NB15-binary"

    save_npz(out_npz, Xadv_raw, Xadv_std, cols, method_name, dataset_name, std, std_path, ckpt_dir, chk)
    print(f"[Saved] {out_npz}")

    summary = {
        "variant": vname,
        "npz": os.path.abspath(out_npz),
        "lambda_stp": float(cfg_v["lambda_stp"]),
        "lambda_corr": float(cfg_v["lambda_corr"]),
        "lambda_mmt": float(cfg_v["lambda_mmt"]),
        "aux_prob": float(cfg_v["aux_prob"]),
        "stdcheck_mean_abs": float(chk),
    }
    return summary
...
def run_one_seed(seed: int):
    # seed-local cfg (avoid cross-seed leakage)
    cfg0 = copy.deepcopy(CFG)
    cfg0["seed"] = int(seed)

    set_seed(int(seed))

    # keep seeds separated
    out_root_seed = f"{cfg0['out_root']}_seed{seed}"
    run_root = make_output_dir(out_root_seed)

    # load canonical cols + shared standardizer
    meta_path = os.path.join(cfg0["artifact_dir"], cfg0["meta_name"])
    std_path = os.path.join(cfg0["artifact_dir"], cfg0["std_name"])
    ...
    Xb_std, Xm_std_train, _, _ = load_ben_mal_chunked_sharedstd(
        cfg0["csv_path"], cfg0["label_col"],
        feat_cols=cols, std=std,
        n_ben=int(cfg0["n_ben"]),
        n_mal=int(cfg0["n_mal_train"]),
        seed=int(seed),
        chunksize=int(cfg0["chunksize"]),
    )

    # feature blocks
    idxT, idxS, idxP = split_feature_blocks_generic(cols)

    # benign train/val split (seed-specific)
    n_b = len(Xb_std)
    n_val = int(n_b * float(cfg0["val_ben_frac"]))
    ...
    # optionally share the same malicious conditioning across variants for fairness (within this seed)
    Xm_raw_gen_shared = None
    if bool(cfg0.get("share_gen_conditioning_across_variants", True)):
        Xm_raw_gen_shared = load_malicious_raw_for_gen(
            cfg0["csv_path"], cfg0["label_col"], feat_cols=cols,
            n_mal=int(cfg0["n_mal_gen"]),
            seed=int(seed),
            chunksize=int(cfg0["chunksize"]),
        )
        print(f"[GenCond] Using shared Xm_raw_gen across variants: {Xm_raw_gen_shared.shape}")

    # run variants
    summaries = []
    ablation_manifest = {}
    for v in cfg0["ablations"]:
        s = run_one_variant(
            run_root=run_root,
            base_cfg=cfg0,
            variant=v,
            ...
            std_path=std_path,
            Xb_std=Xb_std,
            Xm_std_train=Xm_std_train,
            Xb_tr=Xb_tr,
            Xb_val=Xb_val,
            idxT=idxT, idxS=idxS, idxP=idxP,
            Xm_raw_gen_shared=Xm_raw_gen_shared
        )
        summaries.append(s)
        ablation_manifest[s["variant"]] = s["npz"]

    # write per-seed manifest for metrics script
    with open(os.path.join(run_root, "ablation_manifest.json"), "w", encoding="utf-8") as f:
        json.dump({"run_root": run_root, "seed": int(seed), "paths": ablation_manifest}, f, indent=2, ensure_ascii=False)

    with open(os.path.join(run_root, "ablation_summary.json"), "w", encoding="utf-8") as f:
        json.dump({"run_root": run_root, "seed": int(seed), "variants": summaries}, f, indent=2, ensure_ascii=False)

    print("\n" + "="*90)
    print(f"[OK] Seed={seed} finished.")
    print(f"[OK] Wrote: {os.path.join(run_root, 'ablation_manifest.json')}")
    print("="*90)
...
def main():
    # run all seeds and keep a top-level manifest
    ...
    for s in seeds:
        rr = run_one_seed(s)
        all_runs.append({"seed": s, "run_root": rr})

    out_top = ensure_dir(CFG["out_root"])
    top_manifest = os.path.join(out_top, "all_seeds_runs.json")
    with open(top_manifest, "w", encoding="utf-8") as f:
        json.dump({"dataset": "UNSW-NB15-binary", "runs": all_runs}, f, indent=2, ensure_ascii=False)

    print("\n" + "="*90)
    print("[OK] All seeds finished.")
