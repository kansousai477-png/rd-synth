...
CFG = {
    ...
    "n_ben": 300000,
    "n_mal_train": 80000,     # malicious used for conditioning training
    "n_mal_gen": 40000,       # malicious count to generate Xadv for

    # ✅ shared canonical columns + standardizer (must match metrics_suite)
    "artifact_dir": r"artifacts/baselines_light_cic2017_v2",   # contains meta.json + standardizer.npz
    "meta_name": "meta.json",
...
def load_ben_mal_chunked_sharedstd(
    ...
    Return Xb_std, Xm_std, and also benign std min/max for clipping.
    """
...
# =====================================================
# Feature blocks (CIC-IDS-2017 aware)
# =====================================================
def split_feature_blocks_cic2017(cols):
    """
    CICFlowMeter naming-aware split into:
      T: time-related
      S: size/rate/volume-related
      P: protocol/header/flags/ports/window-related
    """
    ...
    size_keys = [
        # lengths / volumes
        "packet_length", "pkt_len", "length", "total_length",
        "bytes", "packets", "payload",
        "fwd_packets", "bwd_packets",
        "fwd_seg_size", "bwd_seg_size", "segment_size",

        # rates
        "flow_bytes/s", "flow_packets/s", "bytes/s", "packets/s",
        "fwd_packets/s", "bwd_packets/s",

        # statistics keywords (CIC features often contain these)
        "mean", "std", "variance", "var", "min", "max", "avg", "average",
        "bulk", "subflow", "rate",
    ]
...
def main():
    ...
    idxT, idxS, idxP = split_feature_blocks_cic2017(cols)
    ...
    chk = np.mean(np.abs(std.transform(Xadv_raw) - Xadv_std))
    ...
    np.savez(
        ...
        # meta is small, keep it simple (pickle OK)
        meta={
